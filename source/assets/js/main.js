...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Modal Mapa
    on('.js-open-modal-map', 'click', (e) => {
        let tipo = e.currentTarget.getAttribute('data-modal-target');
        document.querySelector(`[data-modal-mapa=${tipo}]`).classList.add('active');
        window.location.hash = "modal";
        e.preventDefault();
    })
    on('.js-close-modal-map', 'click', (e) => {
        e.currentTarget.closest('.modal-mapa').classList.remove('active');
        history.pushState(null, null, '/');
        e.preventDefault();
    })

    window.onpopstate = function () {
        let open_modal = document.querySelector('.modal-mapa.active');
